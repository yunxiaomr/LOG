...
class TextRankerReader(DatasetReader):
    ...
    def json_to_instance(self, instance_dict: Dict) -> Instance:
        ...
        for context in contexts:
            ...
            ques = context["prefix_text"].strip()
            tokens_q = ques.split(" ")[:self._max_tokens]
            question_text = " ".join(tokens_q)
            questions.append(question_text)

        return self.text_to_instance(texts, labels, questions, metadata)
    ...
    def text_to_instance(
        ...
        for question in questions:
            ...
            tokens_1 = self._tokenizer.tokenize(question)[: self._max_tokens]
            if self._is_transformer:
                tokens_1 = self._tokenizer.add_special_tokens(tokens_1)
            questions_field = TextField(tokens_1)
            questions_fields.append(questions_field)

        if not questions_fields: # In mixed-answerable set there can be some with no context.
            questions_fields = [TextField(self._tokenizer.tokenize("dummy_1")).empty_field()]

        fields["questions"] = ListField(questions_fields)
        if self._is_transformer:
    ...
    def apply_token_indexers(self, instance: Instance) -> None:
        ...
        for questions_field in instance.fields["questions"]:  # type: ignore
            questions_field._token_indexers = self._token_indexers
