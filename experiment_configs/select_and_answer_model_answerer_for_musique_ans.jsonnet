...
local question_key = "composed_question_text";#问题的键
local max_document_tokens = 4000;
local max_paragraph_tokens = 400;
local max_question_tokens = 150;
local take_predicted_topk_contexts = 4;#选择的上下文数量
...
local train_data_path =
    if setting == 'full'
        then 'serialization_dir/select_and_answer_model_selector_for_musique_ans/predictions/true_filtered_musique_ans_train.jsonl'
    else if setting == 'fixture'
        then fixture_path
    else '-';
local validation_data_path =
    if setting == 'full'
        then 'serialization_dir/select_and_answer_model_selector_for_musique_ans/predictions/true_filtered_musique_ans_dev.jsonl'
    else if setting == 'fixture'
        then fixture_path
    else '-';
