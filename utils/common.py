...
def translate_id_back(key: str) -> str:
    # Naming convention change back to original question-ids.
    namechange = {
        "2hop": "double", "3hop1": "triple_ii",
        "3hop2": "triple_io", "4hop1": "quadruple_iii",
        "4hop2": "quadruple_iot", "4hop3": "quadruple_ioh1", "4hop4": "quadruple_ioh2"
    }
    assert sum([new in key for new, original in namechange.items()]) == 1
    for new, original in namechange.items():
        key = key.replace(new, original)
    return key

def step_placeholder_back(index: int, is_prefix: bool, strip: bool = False) -> str:
    if is_prefix:
        placeholder = f"{CONSTITUENT_QUESTION_START} {index} {CONSTITUENT_QUESTION_END} "
    else:
        placeholder = f"{REPLACEMENT_QUESTION_START} {index} {REPLACEMENT_QUESTION_END} "
